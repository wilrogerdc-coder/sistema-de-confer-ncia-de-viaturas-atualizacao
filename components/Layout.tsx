...
const Layout: React.FC<LayoutProps> = ({ 
  ...
  const unitLabel = useMemo(() => {
    ...
    if (user.scopeLevel === 'POSTO') unitName = postos.find(p => p.id === user.scopeId)?.name || 'POSTO';
    else if (user.scopeLevel === 'SGB') unitName = subs.find(s => s.id === user.scopeId)?.name || 'SGB';
    else if (user.scopeLevel === 'GB') unitName = gbs.find(g => g.id === user.scopeId)?.name || 'GB';
    ...
    <div className="min-h-screen flex flex-col md:flex-row font-sans selection:bg-[var(--theme-primary)] selection:text-white" style={{ backgroundColor: 'var(--theme-bg)' }}>
      <aside className="hidden md:flex w-64 text-white flex-col sticky top-0 h-screen overflow-y-auto z-50 shadow-2xl transition-colors duration-300" style={{ backgroundColor: 'var(--theme-secondary)' }}>
        <div className="p-6 border-b border-white/10">
          ...
          <p className="text-[10px] text-white/50 mt-2 uppercase tracking-widest font-semibold pl-1">{unitLabel}</p>
        </div>
        <nav className="flex-1 p-4 space-y-1">
          {tabs.map(tab => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group ${activeTab === tab.id ? 'text-white shadow-lg shadow-black/20' : 'text-white/60 hover:bg-white/10 hover:text-white'}`}
        ...
        <div className="p-6 border-t border-white/10 bg-black/10 space-y-4">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-lg bg-white/10 flex items-center justify-center text-sm font-bold border border-white/10 text-white/80">{user.name.charAt(0)}</div>
          ...
          <button onClick={onLogout} className="w-full py-2.5 px-4 rounded-lg bg-white/5 hover:bg-red-950/50 hover:text-red-400 text-[10px] font-bold uppercase tracking-wider transition-colors border border-white/10 text-white/70">Encerrar Sessão</button>
        </div>
      </aside>
      <header className="md:hidden border-b border-slate-200 p-4 sticky top-0 z-40 flex justify-between items-center shadow-sm" style={{ backgroundColor: 'var(--theme-surface)' }}>
        <h1 className="text-lg font-bold flex items-center gap-2" style={{ color: 'var(--theme-text-main)' }}><span style={{ color: 'var(--theme-primary)' }}>🚒</span> {APP_NAME}</h1>
        <button onClick={onLogout} className="w-8 h-8 rounded-full flex items-center justify-center border" style={{ backgroundColor: 'var(--theme-bg)', borderColor: 'var(--theme-bg)', color: 'var(--theme-text-main)' }}><span className="text-xs">✕</span></button>
      </header>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 backdrop-blur-md border-t border-slate-200 px-2 py-2 flex justify-around items-center z-50 shadow-[0_-8px_30px_rgb(0,0,0,0.04)]" style={{ backgroundColor: 'var(--theme-surface)' }}>
        {tabs.map(tab => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id)}
            className={`flex flex-col items-center justify-center py-2 px-1 rounded-2xl min-w-[64px] transition-all duration-300 ${activeTab === tab.id ? 'scale-105' : 'grayscale'}`}
